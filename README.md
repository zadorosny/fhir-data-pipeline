...
cp .env.example .env
python -c "import base64, os; print(base64.urlsafe_b64encode(os.urandom(32)).decode())"  # Fernet key
openssl rand -hex 32
...
pytest -m "not integration"                     # unit tests (92% coverage em etl/lib)
ruff check . && ruff format --check . && mypy etl  # lint + format + types
...
├── tests/                               # testes unit + integration (pytest)
