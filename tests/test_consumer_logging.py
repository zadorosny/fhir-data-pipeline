...
from etl.lib.fhir import FHIRError, build_session
...
def test_log_de_falha_usa_cpf_hash_sem_vazar_plaintext(
    ...
    with (
        caplog.at_level(logging.ERROR, logger="test-consumer-err"),
        pytest.raises(FHIRError) as exc_info,
    ):
        process_patient(session, FHIR_BASE, PATIENT_DATA, logger)

    # Diagnosticos preservados para o caller anexar aos headers da DLQ.
